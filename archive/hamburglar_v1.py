...
parser.add_argument("-g", "--git", action="store_true", help="sets hamburglar into git mode")
parser.add_argument("-x", "--hexdump", action="store_true", help="give hexdump of file")
parser.add_argument("-v", "--verbose", help="increase output verbosity", action="store_true")

parser.add_argument(
    "-w",
    "--web",
    help="sets Hamburgler to web request mode, enter url as path",
    action="store_true",
)
parser.add_argument("-i", "--ioc", help="uses iocextract to parse contents", action="store_true")
parser.add_argument("-o", "--out", dest="output", help="write results to FILE", metavar="FILE")

parser.add_argument("-y", "--yara", dest="yara", help="use yara ruleset for checking")
            ...
            if whitelistOn and _iswhitelisted(filepath):
                ...
                try:
                    print(
                        ...
                        + str(filepath)
                        + " ("
                        + str(os.stat(filepath).st_size >> 10)
                        + "kb) to stack"
                    )
                    filestack.add(filepath)
...
def _file_read():
    ...
    while filestack:  # while there are still items on the stack/worker pool...
        ...
        try:
            with open(filepath) as scanfile:  # open file on stack that needs sniffed
...
def _write_to_file(fname):
    ...
    with open(fname, "w") as file:
        file.write(
            json.dumps(dict(cumulativeFindings), default=lambda x: str(x), sort_keys=True, indent=4)
        )
...
def scanGitRepo(target_path, ruleSet=None, yara=False):
    ...
    if yara:
        ...
        print(
            "[+] adding: tmp_git_log"
            + " ("
            + str(os.stat("tmp_git_log").st_size >> 10)
            + "kb) to stack"
        )
