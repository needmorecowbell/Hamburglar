...
class TestScanWebCommand:
    ...
    def test_scan_web_finds_secrets(
        self, mock_web_scan_result: ScanResult
    ) -> None:
        ...
        with patch("hamburglar.cli.main.asyncio.run") as mock_run:
            ...
            result = runner.invoke(
                app, ["scan-web", "https://example.com", "--format", "json"]
            )
            assert result.exit_code == 0
    ...
    def test_scan_web_no_findings(
        self, mock_web_scan_no_findings: ScanResult
    ) -> None:
...
class TestScanWebIncludeScriptsOption:
    ...
    def test_include_scripts_default(
        self, mock_web_scan_result: ScanResult
    ) -> None:
...
class TestScanWebAuthOption:
    ...
    def test_auth_with_valid_format(
        self, mock_web_scan_result: ScanResult
    ) -> None:
    ...
    def test_auth_with_password_containing_colon(
        self, mock_web_scan_result: ScanResult
    ) -> None:
...
class TestScanWebRespectRobotsOption:
    ...
    def test_respect_robots_default(
        self, mock_web_scan_result: ScanResult
    ) -> None:
...
class TestScanWebOutputFormats:
    ...
    def test_table_format(self, mock_web_scan_result: ScanResult) -> None:
        ...
        with patch("hamburglar.cli.main.asyncio.run") as mock_run:
            ...
            result = runner.invoke(
                app, ["scan-web", "https://example.com", "--format", "table"]
            )
            assert result.exit_code == 0
    ...
    def test_invalid_format_fails(self) -> None:
        ...
        result = runner.invoke(
            app, ["scan-web", "https://example.com", "--format", "xml"]
        )
        assert result.exit_code == 1
...
class TestScanWebOutputFile:
    ...
    def test_output_to_file(
        self, mock_web_scan_result: ScanResult, tmp_path: Path
    ) -> None:
...
class TestScanWebQuietMode:
    ...
    def test_quiet_suppresses_output(
        ...
        with patch("hamburglar.cli.main.asyncio.run") as mock_run:
            ...
            result = runner.invoke(
                app, ["scan-web", "https://example.com", "--quiet"]
            )
            assert result.exit_code == 0
...
class TestScanWebVerboseMode:
    ...
    def test_verbose_shows_details(
        ...
        with patch("hamburglar.cli.main.asyncio.run") as mock_run:
            ...
            result = runner.invoke(
                app, ["scan-web", "https://example.com", "--verbose"]
            )
            assert result.exit_code == 0
...
class TestScanWebStreamingMode:
    ...
    def test_stream_option_accepted(self) -> None:
        ...
        with patch("hamburglar.cli.main.asyncio.run") as mock_run:
            ...
            result = runner.invoke(
                app, ["scan-web", "https://example.com", "--stream"]
            )
